      ...
      {!isLoading && filteredPosts.length > 0 && (
        <section className="mb-16">
          <div className="border-4 border-black shadow-neo bg-white flex flex-col md:flex-row overflow-hidden">
             {/* Image container */}
             <div className="w-full md:w-1/2 relative h-[350px] md:h-auto border-b-4 md:border-b-0 md:border-r-4 border-black group cursor-pointer" onClick={() => handleNavigateToPost(filteredPosts[0])}>
                <img 
             ...
             <div className="w-full md:w-1/2 p-6 md:p-10 lg:p-14 flex flex-col justify-center items-start">
               <h1 
                  ...
                  onClick={() => handleNavigateToPost(filteredPosts[0])}
               >
        ...
        {isLoading ? (
          ...
          filteredPosts.map(post => (
            <NeoCard key={post.id} className="h-full flex flex-col" noPadding>
               <div className={`h-48 border-b-4 border-black relative overflow-hidden group cursor-pointer`} onClick={() => handleNavigateToPost(post)}>
               ...
               <div className="p-6 flex flex-col flex-grow bg-white">
                  ...
                  <h3 className="font-display text-2xl uppercase leading-tight mb-4 flex-grow hover:text-rap-blue cursor-pointer transition-colors" onClick={() => handleNavigateToPost(post)}>
                    {post.title}
                  </h3>
                  <div className="mt-auto pt-4 border-t-2 border-gray-100 flex justify-between items-center">
                     ...
                     <button onClick={() => handleNavigateToPost(post)} className="font-bold uppercase text-sm hover:underline flex items-center gap-1">
            ...
            <div className="mt-16">
                 ...
                 <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                    {relatedPosts.map(p => (
                        <div 
                            key={p.id} 
                            onClick={() => handleNavigateToPost(p)}
